...
export function checkHourFormat(hour: string, format: HourFormat = '12') {
  return format === '12'
    ? HOUR_FORMAT_12.test(hour)
    : HOUR_FORMAT_24.test(hour);
}

export function checkMinuteFormat(minute: string) {
  return MINUTE_FORMAT.test(minute);
}

export function checkSecondFormat(second: string) {
